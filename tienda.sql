...
SELECT * FROM fabricante f JOIN producto p ON f.codigo = p.codigo_fabricante WHERE f.nombre = 'Asus' AND p.precio > (SELECT AVG(p.precio) FROM fabricante f JOIN producto p ON f.codigo = p.codigo_fabricante WHERE f.nombre = 'Asus');